...
export default function Footer() {
  ...
  const linksEmpresa = [
    { label: "Sobre Nós", href: "/sobre", title: `Conheça o ${settings.company_short_name}`, isPage: true },
  ...
  const whatsappDisplay = formatWhatsApp(settings.whatsapp || "");
  const whatsappLink = `https://wa.me/${settings.whatsapp || "5561986161961"}?text=${encodeURIComponent("Olá! Gostaria de informações sobre a Ata de Registro de Preço.")}`;
      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 pb-12 border-b border-white/10">
          ...
          <div className="lg:col-span-1">
            <Link
              ...
              aria-label={`${settings.company_short_name} - Ir para o topo`}
            ...
            <ul className="space-y-3" role="list">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-white/60 text-sm">
                    {settings.address_street && (
                      <>
                        {settings.address_street}{settings.address_number ? `, ${settings.address_number}` : ""}
                        <br />
                      </>
                    )}
                    {settings.address_neighborhood && (
                      <>
                        {settings.address_neighborhood} - {settings.address_city}/{settings.address_state}
                        <br />
                      </>
                    )}
                    {!settings.address_neighborhood && settings.address_city && (
                      <>
                        {settings.address_city}/{settings.address_state}
                        <br />
                      </>
                    )}
                    {settings.address_zipcode && `CEP: ${settings.address_zipcode}`}
                  </span>
                </li>
              )}
              {settings.phone && (
                <li className="flex items-center gap-3">
                  ...
                  <a
                    ...
                    title={`Ligar para o ${settings.company_short_name}`}
                  >
                    {settings.phone}
                  </a>
                </li>
              ...
              {settings.email && (
                <li className="flex items-center gap-3">
                  ...
                  <a
                    href={`mailto:${settings.email}?subject=Informações sobre Ata de Registro de Preço`}
                    className="text-white/60 hover:text-[#b6c72c] text-sm transition-colors"
        ...
        <div className="pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-white/40 text-sm text-center md:text-left">
            © {currentYear} {settings.company_short_name}. Todos os direitos reservados.
          </p>
          <div className="flex items-center gap-6 flex-wrap justify-center">
            <span className="text-white/40 text-sm">CNPJ: {settings.cnpj}</span>
            <Link
